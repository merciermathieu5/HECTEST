...
(async () => {
  ...
  console.log('— Recote probable recentrée sur la tendance du profil (tableaux d\'analyse)');
  // Tendances transcrites du document, non inventées
  proche(S.TENDANCE_PROFIL.SNIPER, -0.76, 1e-9, 'Tendance Sniper = -0,76 OV (cohorte de la ligue)');
  proche(S.TENDANCE_PROFIL.ELITE, 0.12, 1e-9, 'Tendance Elite = +0,12 OV');
  proche(S.TENDANCE_PROFIL.DEFFWD, -1.30, 1e-9, 'Tendance Defensive = -1,30 OV');
  egal(S.ECHANTILLON_PROFIL.DDEPTH, 3, 'Échantillon Ddepth Defense = 3 (faible, prudence)');
  egal(S.ECHANTILLON_PROFIL.ELITE, 112, 'Échantillon Elite = 112');
  // Un Sniper "Correct" (ModS 0) hérite quasi de la tendance de sa cohorte
  const sniper27 = {po:'AD', age:27, _profil:{mat:'SNIPER', profil:'Sniper'}}; // ModA 0
  proche(S.recoteProbable(sniper27, 0), -0.76, 1e-9,
    'Sniper 27 ans "Correct" → probable ≈ tendance cohorte (-0,76)');
  // Une saison "Mémorable" (ModS 60) reste positive malgré une cohorte en baisse
  const probMemo = S.recoteProbable(sniper27, 60);
  ok(probMemo > 0, 'Sniper "Mémorable" reste positif (+perf domine la tendance négative)');
  proche(probMemo, -0.76 + (60/50)*1.2, 1e-9, 'Probable = tendance + performance (ModA 0)');
  // Le bonus d'âge nuance : un jeune Elite "Correct" profite du ModA positif
  const jeuneElite = {po:'C', age:20, _profil:{mat:'ELITE', profil:'Prospect Elite'}}; // ModA +25
  proche(S.recoteProbable(jeuneElite, 0), 0.12 + 0.25, 1e-9,
    'Jeune Elite "Correct" → tendance +0,12 + bonus d\'âge +0,25');
  // Bornage à l'échelle réglementaire
  const vieux = {po:'C', age:40, _profil:{mat:'DEFFWD', profil:'Defensive'}}; // ModA -50
  ok(S.recoteProbable(vieux, -40) >= -5, 'Probable jamais sous -5 (plancher réglementaire)');
  egal(S.recoteProbable(kotka, null), null, 'Aucune stat évaluée → probable null');
